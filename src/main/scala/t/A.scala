...
object A extends App {
  ...
  Database.forURL("jdbc:h2:mem:test1", driver = "org.h2.Driver") withDynSession {
    ...
    val q2 = Q.query[(Double, Double), (String, String)]( """
  select c.name, s.name
  from coffees c, suppliers s
  where c.price < ? and c.price < ? and s.id = c.sup_id""")
    // This time we read the result set into a List
    val l2 = q2.list(9.0, 100)
    ...
    Q.queryNA[Coffee]("select * from coffees") foreach {
      c =>
